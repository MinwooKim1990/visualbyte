...
CONFIG = {
    ...
    "save_path": "best_visual_byte_train_unfreeze_convnext_v5.pth",
    "save_path_last": "last_visual_byte_train_unfreeze_convnext_v4.pth",
    "cap_per_class": 8,
    # 1: whisper encoder map (recommended), 2: simple log-mel
    "audio_image_convert_method": 1,
    # audio loss weighting
    "audio_loss_weight": 1.0,   # base lambda for audio losses
    "warmup_audio": False,       # if True, use 3-phase schedule over epochs
    "save_after_warmup": True,
    # eval settings (for metric-based checkpoint)
    "eval_audio_root": "data/audio_eval_tts",
    "eval_cap_per_class": 1,
    "eval_batch_size": 64,
}
...
class VisualByteDataset(Dataset):
    def __init__(
        ...
        if self.text_image_root and self.text_image_root.exists():
            for local_idx, (en, ko, slug) in enumerate(classes):
                ...
                if p.exists():
                    self.text_image_dict[local_idx] = p
            print(f"✔ Using precomputed text images from {self.text_image_root} ({len(self.text_image_dict)} classes)")
    ...
    def __getitem__(self, idx):
        ...
        # text image: precomputed if available, else texture factory
        if local_idx in self.text_image_dict:
            try:
                with Image.open(self.text_image_dict[local_idx]).convert("RGB") as im:
...
def compute_recall(eval_loader, model, device):
    model.eval()
    gallery_feats = []
    gallery_lbls = []
    txt_feats = []
    ...
    with torch.no_grad():
        for b in eval_loader:
            ...
            texts = b["text"].to(device)
            audios = b["audio"].to(device)
            lbls = b["label"].to(device)
            has_audio = b["has_audio"].to(device)

            li, _ = model(imgs)
            lt, _ = model(texts)
            gallery_feats.append(li.cpu())
            gallery_lbls.append(lbls.cpu())
            txt_feats.append(lt.cpu())
            txt_lbls.append(lbls.cpu())

            if has_audio.any():
                la, _ = model(audios)
                mask = has_audio > 0
                aud_feats.append(la[mask].cpu())
                aud_lbls.append(lbls[mask].cpu())

    gallery_feats = torch.cat(gallery_feats)
    gallery_lbls = torch.cat(gallery_lbls)
    nn_model = NearestNeighbors(n_neighbors=5, metric="cosine").fit(gallery_feats.numpy())

    def r_at_k(feats_list, lbls_list, k):
        if len(feats_list) == 0:
            return 0, 0
        feats = torch.cat(feats_list).numpy()
        lbls = torch.cat(lbls_list).numpy()
    ...
    t_hit1, t_tot = r_at_k(txt_feats, txt_lbls, k=1)
    t_hit5, _ = r_at_k(txt_feats, txt_lbls, k=5)
    a_hit1, a_tot = r_at_k(aud_feats, aud_lbls, k=1)
    a_hit5, _ = r_at_k(aud_feats, aud_lbls, k=5)
    return {
        "text_r1": (t_hit1, t_tot),
        "text_r5": (t_hit5, t_tot),
        "audio_r1": (a_hit1, a_tot),
        "audio_r5": (a_hit5, a_tot),
    }
...
if __name__ == "__main__":
    ...
    # eval dataset/loader (use old audio_img_eval_{method})
    eval_auto_audio_image_root = Path(f"data/audio_img_eval_{CONFIG['audio_image_convert_method']}")
    eval_audio_img_root = str(eval_auto_audio_image_root) if eval_auto_audio_image_root.exists() else None
    eval_ds = EvalDataset(
        classes,
        CONFIG["eval_audio_root"],
        image_size=CONFIG["image_size"],
        cap_per_class=CONFIG.get("eval_cap_per_class", 1),
        audio_method=CONFIG["audio_image_convert_method"],
        audio_image_root=eval_audio_img_root,
        text_image_root=CONFIG.get("text_image_root"),
    )
    eval_loader = DataLoader(eval_ds, batch_size=CONFIG.get("eval_batch_size", 64), shuffle=False, num_workers=2)
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        # evaluate recall
        metrics = compute_recall(eval_loader, model, CONFIG["device"])
        t_hit1, t_tot = metrics["text_r1"]
        a_hit1, a_tot = metrics["audio_r1"]
        t_r1 = (t_hit1 / t_tot * 100) if t_tot > 0 else 0.0
        a_r1 = (a_hit1 / a_tot * 100) if a_tot > 0 else 0.0
        t_r5 = (metrics["text_r5"][0] / t_tot * 100) if t_tot > 0 else 0.0
        a_r5 = (metrics["audio_r5"][0] / a_tot * 100) if a_tot > 0 else 0.0
        score = (t_r1 + a_r1) / 2 if a_tot > 0 else t_r1
        print(f"🔎 Eval R@1 text {t_r1:.1f}% | audio {a_r1:.1f}% | R@5 text {t_r5:.1f}% | audio {a_r5:.1f}% | score {score:.2f}")

        if score > best_score:
            best_score = score
            print(f"✅ Saving Best by score: {score:.2f}")
            torch.save(model.state_dict(), CONFIG["save_path"])

    torch.save(model.state_dict(), CONFIG["save_path_last"])
    print(f"💾 Saved last model to {CONFIG['save_path_last']}")
    print("✨ All Done. Use visual_byte_unfreeze_eval_v4.py for evaluation.")
